...
%% ================================================================================
%  Προσομοίωση Προσαρμοστικού Ελεγκτή Γραμμικοποίησης μέσω Backstepping (Ερώτημα ε)
%  ================================================================================
...
t_span = [0 5]; % Χρονικό διάστημα προσομοίωσης
...
for j = 1:4
    ...
    [t, states] = ode15s(@(t, x) backstepping_sys(x, theta_star, gamma, k1, k2, g, gp), t_span, x0, options);
    ...
    for i = 1:length(t)
        ...
        % Νόμος Ελέγχου u
        u_hist(i) = -z1 - k2*z2 - (k1 + theta_hat(i)*gp(z1))*(x2(i) + theta_hat(i)*g(z1)) - g(z1)*dot_th;
    end
    ...
    plot(t, u_hist, 'm', 'LineWidth', 1.5); grid on;
    title(['Case ', num2str(j), ': Control Input u']);
    if row_idx == 2, xlabel('Time (s)'); end

    % --- Αποθήκευση Figures ---
    if j == 2
        saveas(fig_h, 'E_Cases_1_2.png');
    elseif j == 4
        saveas(fig_h, 'E_Cases_3_4.png');
    end
end
...
% Συνάρτηση Δυναμικής Backstepping
function dxdt = backstepping_sys(x, theta_star, gamma, k1, k2, g, gp)
    x1 = x(1); 
    x2 = x(2); 
    theta_hat = x(3);
